...
class WebworldResultsPage {
    ...
    loadData(data) {
        ...
        this._results = data.assets;
        this._resultKeys = Object.keys(this._results);
        this._cursor = 0;
    ...
    _nextPage() {
        if(this._cursor + this._previews.length <= this._resultKeys.length) {
            global.pointerInteractableManager.removeInteractables(this._interactables);
            this._cursor += this._previews.length;
            this._updateMenu();
            global.pointerInteractableManager.addInteractables(this._interactables);
        ...
        for(let i = 0; i < this._previews.length; i++) {
            ...
            if(index < this._resultKeys.length) {
                ...
                if(index < this._previewTextures.length) {
                    ...
                    let previewUrl = this._getPreviewUrl(this._resultKeys[index]);
    ...
    _getPreviewUrl(assetId) {
        let previewUrl = global.assetsMap[assetId].smallPreviewImage;
        return previewUrl;
    }
    ...
    _goToPage(assetId) {
        let data = {
            assetId: assetId,
            instances: this._results[assetId],
            webworldId: this._webworldId
        }
        let page = this._controller.getPage(HomeSceneMenus.WEBWORLD_ASSET);
        page.loadData(data);
        this._controller.goToPage(HomeSceneMenus.WEBWORLD_ASSET);
    }
...
export default WebworldResultsPage;
